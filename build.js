...
async function build() {
  ...
  for (let level = 1; level < gradeByKanjis.length - 1; level++) {
    const info = {};
    for (const kanji of gradeByKanjis[level]) {
      // TODO: Onkun にノイズが多い
      let [kun, on] = onkunDict.get(kanji);
      let onkun = [];
      if (kun) onkun = onkun.concat(kun);
      if (on) {
        on = on.map((hira) => hiraToKana(hira));
        onkun = onkun.concat(on);
      }
      info[kanji] = [];
      ...
      const vocab = await getGradedVocab(kanji, level);
      const idioms = await getGradedIdioms(kanji, level);
      words.push(...original);
      words.push(...vocab);
      words.push(...idioms);
